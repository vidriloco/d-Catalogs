...
feature 'Discounts management (for items)' do
  ...
  describe "Visiting an item description page with discounts", :js => true do
    
    before(:each) do
      @discount = Fabricate(:discount, :discountable => @item)
      visit item_path(@item)
      find("#discounts-tab").click
    end
    ...
    scenario "then I can modify it" do
      ...
      page.current_path.should == edit_item_discount_path(@item, @discount)
